...
/** `?area=agents` (or `connectors`/`jobs`) opens straight there — a deep
 *  link for previews, the same idea as `?signedin=1` above. */
function startingArea(): "chat" | "agents" | "connectors" | "jobs" {
  if (typeof window === "undefined") return "chat";
  const a = new URLSearchParams(window.location.search).get("area");
  return a === "agents" || a === "connectors" || a === "jobs" ? a : "chat";
}
...
export function ConductorApp({
  ...
  */
  ...
  const [area, setArea] = useState<"chat" | "agents" | "connectors" | "jobs">(startingArea);
