          ...
          return (
            <div key={c.id} className={`cn-card${expanded ? " cn-card--open" : ""}`}>
              ...
              >
                ...
                {/* Who is using it, right on the row — the fact most likely to
                    change what someone does next. */}
                <span className="cn-users">
                  {users.length === 0 ? (
                    <span className="cn-unused">No agents yet</span>
                  ) : (
                    users.map((a) => (
                      <span key={a.id} className="cn-user">
                        <AgentOrb status={a.status} size={6} accent={a.accent} />
                        {a.name}
                      </span>
                    ))
                  )}
