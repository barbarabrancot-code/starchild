      ...
      >
        <div className="ap-rows">
          ...
          {found.map((a, i) => (
            ...
            >
              <span className="ap-glyph"><AgentOrb status={a.status} size={11} accent={a.accent} /></span>
              <span className="ap-name">{a.name}</span>
              <span className="ap-mood">{a.mood}</span>
            </button>
          ))}
        ...
        {/* Only keys that do something. A hint for a shortcut the product does not
            have costs more trust than the shortcut would have saved. */}
        <div className="ap-keys">
          <span><kbd>↑</kbd><kbd>↓</kbd> move</span>
          <span><kbd>⏎</kbd> {creating ? "create" : "open"}</span>
