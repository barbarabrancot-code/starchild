...
export function ActiveTaskCard({
  ...
  const found = Boolean(task.activity);

  // A suggestion has a decision sitting on it and has to be seen in full to be
  // made. A standing task does not — most of the time it is just proof the
  // Chief Agent is still holding the rule, which a title and a status already
  // say. So it starts as a line you can read the roster in, not a card you
  // have to read past.
  const [open, setOpen] = useState(suggesting);
    ...
    return (
      <>
        <motion.button
          ...
          className={`tcard tcard-mini${paused ? " tcard--off" : ""}${found ? " tcard--found" : ""}`}
        >
          <span className="tcard-mini-name">{task.title}</span>
          <span className="tcard-state">{TASK_STATUS_LABEL[task.status]}</span>
        </motion.button>
        <TaskCardStyle />
      </>
    );
  ...
  return (
    ...
    >
      <div className="tcard-head">
        ...
        {!suggesting && (
          <button type="button" className="tcard-collapse" onClick={() => setOpen(false)} aria-label="Collapse">
...
function TaskCardStyle() {
  return (
    <style>{`
      ...
      /* The collapsed line: same family, one row, nothing to read past. It is a
         button rather than a static row because opening it back up is the whole
         point of leaving it this small. */
      .tcard-mini {
        flex-direction: row; align-items: center; gap: 10px;
        padding: 9px 14px; cursor: pointer; text-align: left;
        transition: background-color .15s ease;
      }
      .tcard-mini:hover { background: rgba(255,255,255,.06); }
      .tcard-mini-name {
        min-width: 0; flex: 1; font-size: 13px; font-weight: 600; color: #fff;
      ...
      .tcard-collapse {
        ...
        color: rgba(255,255,255,.38); transform: rotate(180deg);
        transition: background-color .15s ease, color .15s ease;
      }
      .tcard-collapse:hover { background: rgba(255,255,255,.08); color: rgba(255,255,255,.8); }
