...
export function AgentOrb({
  ...
  halo = false,
  /** the roster's rows are a list to scan, not a room to feel alive in — no
   *  breath, no drift, no beat. Status still reads through form and colour. */
  still = false,
}: {
  status: AgentStatus;
  size?: number;
  ...
  const { ref, controller } = usePresenceDot<HTMLSpanElement>({
    ...
    breath: still ? 0 : BREATH[status],
  });

  // Working is the only one that wanders. Everything else holds its place, which
  // is what makes the wandering mean something.
  useDriftAim(controller, status === "working" && !still, 1.5, reduced);

  useEffect(() => {
    if (status === "working" && !still) return;
    controller.aim(0, 0);
  }, [controller, status, still]);
