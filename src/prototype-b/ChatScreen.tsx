...
type TailBody =
  ...
  /** No Jobs area in this version, so a repeated request becomes a real,
   *  dedicated agent rather than a lightweight standing task — this is its
   *  confirmation, pointing at the roster rather than a page that no longer
   *  exists. */
  | { kind: "agentHandled"; agentId: string };
...
export function ChatScreen({
  ...
  /** set by the Chat/Agents/Connectors shell when a saved conversation is
  ...
  // Grows with what's typed rather than scrolling sideways.
  useEffect(() => {
  ...
  const { isConnected, roster, addAgent, updateAgent, removeAgent, activeTasks, addTask, updateTask } = useAgents();
